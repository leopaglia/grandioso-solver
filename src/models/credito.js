const credito = ({ nombre, capitalTotal, tna, capitalizacion, cuotas, tipo, anioInicial }) => {  
  const tea = Math.pow(1 + tna * capitalizacion / 360, 360 / capitalizacion) - 1

  const creditMapping = {
    frances: {
      cuota: () => capitalTotal * (tea / (1 - Math.pow(1 + tea, -cuotas))),
      interes: restante => restante * tea,
      capital: restante => capitalTotal * (tea / (1 - Math.pow(1 + tea, -cuotas))) - restante * tea
    },
    aleman: {
  ...
  const outOfRange = periodo => periodo < anioInicial || periodo > anioInicial + cuotas
  ...
  return {
    ...
    egAfectImp: periodo => ({
      components: [{ name: `Interes ${nombre}`, value: outOfRange(periodo) ? 0 : valores[periodo - anioInicial].interes }],
      total: outOfRange(periodo) ? 0 : valores[periodo - anioInicial].interes,
    }),
    egNoAfectImp: periodo => ({
      components: [{ name: `Capital ${nombre}`, value: outOfRange(periodo) ? 0 : periodo === anioInicial ? capitalTotal : -valores[periodo - anioInicial].capital }],
      total: outOfRange(periodo) ? 0 : periodo === anioInicial ? capitalTotal : -valores[periodo - anioInicial].capital,
    }),
    inversion: () => ({
      components: [],
      total: 0
