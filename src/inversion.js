const inversion = ({ 
  ...
  const valorLibro = valorTotal - limiteAmortizacion - cuotaDepreciacion * (anioVenta - anioInicial + 1)
  ...
  return {
    ingAfectImp: periodo => ({
      components: [
        ...(perdidaValorAnual ? [{ 
          name: `Venta ${nombre}`, 
          value: perdidaValorAnual && (periodo === anioVenta + 1) ? valorVenta : 0 
        }] : [])
      ],
    ...
    egAfectImp: periodo => ({
      components: [{ 
        ...
        value: periodo === anioVenta + anioInicial - 1 ? costoDesinstalacion : 0,
      }],
    ...
    gastosNoDes: periodo => ({
      components: [{
        ...
        value: (periodo >= anioInicial && periodo <= anioVenta + anioInicial - 1 ? cuotaDepreciacion : 0) 
        + (periodo === anioVenta + anioInicial - 1 ? valorLibro : 0)
      }],
      total: (periodo >= anioInicial && periodo <= anioVenta + anioInicial - 1 ? cuotaDepreciacion : 0) 
             + (periodo === anioVenta + anioInicial - 1 ? valorLibro : 0)
    }),
  }
}
